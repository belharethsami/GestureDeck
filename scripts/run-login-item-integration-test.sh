...
if [[ ! -x "$executable" ]]; then
    print -u2 "Install GestureDeck 0.3.0 in Applications before running this test."
    exit 1
fi
...
if [[ "$installed_version" != "0.3.0" ]]; then
    print -u2 "Expected GestureDeck 0.3.0 in Applications, found $installed_version."
    exit 1
fi
